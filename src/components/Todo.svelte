<script lang="ts">
	...
	function handleEdit(e: KeyboardEvent, id: string) {
	...
	function handleBlur(e: FocusEvent, id: string) {
		let targetElement = e.target as HTMLInputElement
		let newtodo = targetElement.value
	...
	<div class="todo-item">
		<div>
			<input
				...
				class="toggle"
			/>
			<label class="todo-check" for="todo" />
		</div>
		...
		<span on:dblclick={toggleEdit} class:completed={todo.completed} class="todo-text"
			>{todo.text}</span
		>
		<button on:click={() => removeTodo(todo.id)} class="remove"></button>
	</div>
